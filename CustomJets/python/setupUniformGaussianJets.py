...
from SRothman.CustomJets.UniformGaussianJetProducer_cfi import *

def setupGenUniformGaussianJets(process,
                      genjets,
                      chargedOnly,
                      eventSelection,
                      name):
    ...
    setattr(process, 'Gen'+name, GenUniformGaussianJetProducer.clone(
        jetSrc = genjets,
...
def setupRecoUniformGaussianJets(process,
                       jets,
                       CHSjets,
                       chargedOnly,
                       eventSelection,
    ...
    setattr(process, name, PatUniformGaussianJetProducer.clone(
        jetSrc = jets,
        eventSelection = eventSelection,
        doEventSelection = doEventSel,
        CHSsrc = CHSjets,
...
def setupUniformGaussianJets(process, 
                   jets, 
                   genjets,
                   CHSjets,
                   chargedOnly,
                   ...
                   genOnly):
    if isMC:
        process = setupGenUniformGaussianJets(process,
                                    genjets,
                                    chargedOnly,
                                    eventSelection,
                                    name)
    if not genOnly:
        process = setupRecoUniformGaussianJets(process,
                                     jets,
                                     CHSjets,
                                     chargedOnly,
                                     eventSelection,
